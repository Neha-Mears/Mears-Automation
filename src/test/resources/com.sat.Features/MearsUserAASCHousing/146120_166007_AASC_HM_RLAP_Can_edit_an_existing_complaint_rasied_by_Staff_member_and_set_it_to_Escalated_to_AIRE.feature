...
Feature: TC_146120: AASC HM| Can edit an existing complaint rasied by Staff member and set it to Escalated to AIRE
  ...
  Scenario Outline: ser is on the address page of a property with an existing tenancy and existing complaint raised by Staff member which is not at "Escalated to AIRE or Complete" status
   ...
   Then Scroll down and click on expansion panel header button for "<username>"
    ...
    And user click on "Status" the Drop Down List
    And user select "Escalated to AIRE" from dropdown
    Then user click on save button
    ...
    #  | MCMViewHMUsername | MCMAppUserPassword | Portal      | SSSA10 DA  , DA STREET,  Gloucester,  Gloucestershire,  England,  GL13NN | Complaint Status Set to Complete. | Please ensure you have specified an outcome and a resolved date. | August   |    2023 |      18 | The operation was successful |
