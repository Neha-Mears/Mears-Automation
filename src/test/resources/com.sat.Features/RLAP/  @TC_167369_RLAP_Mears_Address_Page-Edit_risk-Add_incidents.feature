...
Feature: RLAP Mears_Address Page - Edit risk - Add incidents
  ...
  Scenario Outline: Make sure User is able to Login
    ...
    Then user fill details for "Details" as "DemotestEditDetails"
    Then user fill details for "Mitigating Factors" as "Emergency factor"
    Then user fill details for "Incidents" as "Immediate"
    Then user click on "Save" button
   # Then Validate the "risks-list__wrapper" is added shows as "risks-list__wrapper" for "DemotestEditDetails"
    Then user click on "risks-list__column" button
    Then user varify the following grids is displayed
      | Details: | Aggravating Factors: | Mitigating Factors: | Incidents: | Review Date: |
    Then user varify the following grids is displayed
      | Demotestformandatoryfield | Stutest | Emergency factor | Immediate | 30/10/2023 |

    Examples: 
      | username      | password          | serviceName | address                                                                  | message1                   | message2                        | setmonth | setyear | setDate | success_msg                  |
      | RLAPMearsuser | RLAPMearspassword | Portal      | SSSA10 DA  , DA STREET,  Gloucester,  Gloucestershire,  England,  GL13NN | Valid details are required | A valid review date is required | October   |    2023 |      30 | The operation was successful |
